...
def ffmpeg_get_iframes(path, start, frames_to_scan=60):
    ...
    while proc.poll() is None:
        ...
        for line in lines:
            fn, fi, parse = Parsers.parse_auto(line)
            if parse is None:
                continue
            if fn == 'showinfo':
                if parse['type'] == 'I':
...
def ffmpeg_cropdetect(url, video_track: MediaFile.VideoTrack, cd_black=0.08, cd_round=4, cd_reset=40, frames=100):
    ...
    for i in range(1, 10):
        ...
        cdyb = 50000
        start0 = int(start - 3)
        command = 'ffmpeg -y -ss {start:.2f} -i {src} -ss 3 -vsync 0 -copyts -vframes {vframes} -filter_complex [0:v:{idx}]showinfo,cropdetect={cd0}:{cd1}:{cd2}[v] -map [v] -f null {nul}' \
                  ''.format(start=start0, src=url, vframes=frames, idx=video_track.index_kind, cd0=cd_black, cd1=2, cd2=cd_reset, nul=DEVNULL)
        Logger.debug('{}\n'.format(command))
        proc = Popen(command.split(' '), stderr=PIPE)
        while proc.poll() is None:
            lines = proc.stderr.readline().decode().split('\r')
...
def test_ffmpeg_create_preview_extract_audio_subtitles():
    ...
    dir_tr = Storage.storage_path(role='transit')
    dir_pr = TRIX_CONFIG.storage.servers[0].get_paths(role='preview')[0]
...
def test_mediafile_asset_for_ingest():
    res = mediafile_asset_for_ingest(FFMPEG_UTILS_TEST_FILE_AV, None)
    pprint(res)
    Logger.log('sleep...\n')
    time.sleep(5)
    Logger.log('done\n')
